...
# --- 2a) PCA with supplementary variables ---
# - Position = qualitative supplementary (index 1)
# - EFF = quantitative supplementary (find its column index)
eff_idx <- which(colnames(df_subset) == "EFF")

res <- PCA(df_subset,
           scale.unit = TRUE,
           quali.sup  = 1,                      # Position as supplementary qualitative
           quanti.sup = eff_idx,                # EFF as supplementary quantitative
           graph = FALSE)
```
...
```{r}
# --- 2b) Eigenvalues / how many components ---
print(res$eig)   # columns: eigenvalue, % of var, cum %
# The loadings of the variables on the components
corrplot(res$var$coord)
# Scree plot. Dependency - library(factoextra)
fviz_eig(res)
```
...
```{r}
# --- 2d) Individual plots (players) ---
# Color individuals by Position for interpretation (habillage = col index of the qualitative sup variable)
plot.PCA(res, choix = "ind", axes = c(1, 2), habillage = 1)
plot.PCA(res, choix = "ind", axes = c(1, 3), habillage = 1)
plot.PCA(res, choix = "ind", axes = c(2, 3), habillage = 1)

# More visually appealing alternative
fviz_pca_ind(res, col.ind = df$Position)
fviz_pca_ind(res, col.ind = df$Position, axes = c(1, 3))
fviz_pca_ind(res, col.ind = df$Position, axes = c(2, 3))
...
The Euclidean distance matrix, based on the scaled numerical variables, has been computed using the *dist()* function. Using this matrix, 



```{r}
ds <- dist(scale(df_subset[,-1]), mathod = "euclidean")
...
Since all the binary variables represent whether each statistic is above or below the mean, and the Position variable is conceptually different from them, it makes sense to perform an MCA using only the binary variables, while treating Position as a supplementary variable.

In this case, since there are 15 binary categories, the maximum number of dimensions is $15$. Following the same reasoning as before, the relevant dimensions are $1$ and $2$.
...
res.mca.extra <- MCA(df_binary, quali.sup=1)
